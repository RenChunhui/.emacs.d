...
(defcustom chunhui-theme-display-vc-status 'full
...
(defface chunhui-ro-face
  '((t :foreground "#0088CC" :weight bold))
  "Face for read-only buffer in the mode-line.")

(defface chunhui-modified-face
  '((t :foreground "#ff6c6b" :height 0.9))
  "Face for modified buffers in the mode-line.")

(defface chunhui-not-modified-face
  '((t :foreground "#98be65" :height 0.9))
  "Face for not modified buffers in the mode-line.")

(defface chunhui-buffer-position-face
  '((t :height 0.9))
  "Face for line/column numbers in the mode-line.")

(defface chunhui-vc-face
  '((t :foreground "#61afef"))
  "Face for vc status in the mode-line.")

(defface chunhui-ok-face
  '((t :foreground "#61afef"))
  "Face for ok status in the mode-line.")

(defface chunhui-warning-face
  '((t :foreground "#da8548"))
  "Face for warning status in the mode-line.")

(defface chunhui-error-face
  '((t :foreground "#ff6c6b"))
  "Face for error status in the mode-line.")

(defvar chunhui-modeline-position '(:eval (propertize ":%l:%c %p " 'face (if (chunhui--active-window-p)
                                                                              'chunhui-buffer-position-face
                                                                            'mode-line-inactive)))
  "Mode line construct for displaying the position in the buffer.")

(defvar chunhui-modeline-buffer-identification '(:eval (propertize "%b" 'face 'bold))
  "Mode line construct for displaying the position in the buffer.")

(defvar chunhui-modeline-modified '(:eval (if (buffer-modified-p (current-buffer))
                                               (all-the-icons-faicon "floppy-o"
                                                                     :height 0.9
                                                                     :v-adjust 0
                                                                     :face (if (chunhui--active-window-p)
                                                                               'chunhui-modified-face
                                                                             'mode-line-inactive))
                                             (all-the-icons-faicon "check"
                                                                   :height 0.9
                                                                   :v-adjust 0
                                                                   :face (if (chunhui--active-window-p)
                                                                             'chunhui-not-modified-face
                                                                           'mode-line-inactive)))))

(defvar chunhui-modeline-ro '(:eval (if buffer-read-only
                                         (if (chunhui--active-window-p)
                                             (progn
                                               (propertize "RO " 'face 'chunhui-ro-face))
                                           (propertize "RO " 'face 'bold))
                                       "")))

(defvar chunhui-buffer-coding '(:eval (unless (eq buffer-file-coding-system (default-value 'buffer-file-coding-system))
                                         mode-line-mule-info)))

(defvar chunhui-modeline-vc '(vc-mode ("   "
                                        (:eval (all-the-icons-faicon "code-fork"
                                                                     :height 0.9
                                                                     :v-adjust 0
                                                                     :face (when (chunhui--active-window-p)
                                                                             (chunhui-git-face))))
                                        (:eval (when (eq chunhui-theme-display-vc-status 'full)
                                                 (propertize (truncate-string-to-width vc-mode 25 nil nil "...")
                                                             'face (when (chunhui--active-window-p)
                                                                     (chunhui-git-face))))))))

(defun chunhui-modeline-flycheck-status ()
                   ...
                   (`finished (if flycheck-current-errors
                                  (let ((count (let-alist (flycheck-count-errors flycheck-current-errors)
                                    ...
                                    (propertize (format "✖ %s Issue%s" count (if (eq 1 count) "" "s"))
                                                'face (chunhui-face-when-active 'chunhui-error-face)))
                                (propertize "✔ No Issues"
                                            'face (chunhui-face-when-active 'chunhui-ok-face))))
                   (`running     (propertize "⟲ Running"
                                             'face (chunhui-face-when-active 'chunhui-warning-face)))
...
(defvar chunhui--git-face-cached (cached-for 1 (chunhui--git-face-intern)))

(defun chunhui--git-face-intern ()
  ...
  (if (magit-git-success "diff" "--quiet")
      ...
      (if (zerop (length (magit-git-string
        ...
        'chunhui-warning-face)
    'chunhui-error-face))

(defun chunhui-git-face ()
  "Return the face to use based on the current repository status.
The result is cached for one second to avoid hiccups."
  (funcall chunhui--git-face-cached))
...
;;;###autoload
(defun chunhui-setup-modeline-format ()
  "Setup the mode-line format for chunhui."
  (interactive)
  (require 'flycheck)
  (require 'magit)
  ...
  (setq-default mode-line-format
                `("%e"
                  " "
                  ,chunhui-modeline-ro " "
                  ,chunhui-buffer-coding
                  mode-line-frame-identification " "
                  " "
                  ,chunhui-modeline-modified
                  " "
                  ,chunhui-modeline-buffer-identification
                  ,chunhui-modeline-position
                  ,(if chunhui-theme-display-vc-status
                       chunhui-modeline-vc
                     "")
                  "  "
                  (:eval (chunhui-modeline-flycheck-status))
